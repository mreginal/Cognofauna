...
function criarFormularioEdicao(dadosUsuario) {
    ...
    const serie = criarCampoInput('text', 'serie', 'serie', 'Serie:', dadosUsuario.serie);
    const botaoEnviar = document.createElement('button');
    botaoEnviar.type = 'submit';
    botaoEnviar.textContent = 'Enviar';

    form.appendChild(nome);
    form.appendChild(datanasc);
    form.appendChild(genero);
    form.appendChild(escola);
    form.appendChild(serie);
...
function criarCampoInput(type, id, name, label, valor) {
    ...
    input.type = type;
    input.id = id;
    input.name = name; // Adicionando o atributo name
    input.value = valor;
...
async function enviarEdicaoUsuario(form) {
    ...
    console.log('Dados do formulário antes:', formData); // Exibe os dados do formulário antes
    ...
    console.log('Dados atualizados antes:', dadosAtualizados); // Exibe os dados atualizados antes de enviar
